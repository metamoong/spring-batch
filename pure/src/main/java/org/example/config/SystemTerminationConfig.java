...
public class SystemTerminationConfig {
    ...
    /**
     * Spring Batch는 Job을 실행할 때 Spring 컨테이너에서 Job 빈을 찾아 실행함
     * -> Job은 반드시 Bean으로 등록
     */
    @Bean
    public Job systemTerminationSimulationJob() {
        // job의 이름과 jobRepository 전달
        // jobRepository 에서 메타 데이터 등 관리
        return new JobBuilder("systemTerminationSimulationJob", jobRepository)
                .start(enterWorldStep()) // step을 순차적으로 정의 -> 이전 step이 성공적으로 완료되어야 다음 step이 실행됨
                .next(meetNPCStep())
